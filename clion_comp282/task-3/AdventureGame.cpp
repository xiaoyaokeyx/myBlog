...
void DescribeExits(Player &player) {
    ...
    //for (auto exit: player.getCurrentLocation().getExits()) {
    //    std::cout << exit.first << " ";
    //}
    std::cout << std::endl;
}
...
int main() {
    ...
    // create map
    cave.addExit('E', &field);   // cave的东面是field
    field.addExit('W', &cave);// field的西面是cave
    field.addExit('E', &forest);// field的东面是forest
    field.addExit('S', &dungeon); // field的南面是dungeon
    ...
    Player player;
    player.setCurrentLocation(&clearing); // player的当前位置是clearing
    ...
    while (true) {
        ...
        if (command == "quit") {
            ...
            if (player.getCurrentLocation().getExits().find(command[0]) !=
                player.getCurrentLocation().getExits().end()) {
                player.setCurrentLocation(player.getCurrentLocation().getExits()[command[0]]);
            } else {
